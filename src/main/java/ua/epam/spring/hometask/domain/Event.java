...
public class Event extends DomainObject {
    ...
    public Event clone(){
        ...
        eventCopy.setName(this.getName());
        eventCopy.setBasePrice(this.getBasePrice());
        eventCopy.setRating(this.getRating());
        NavigableSet<LocalDateTime> airDatesCopy = new TreeSet<>();

        for (LocalDateTime ldt :this.airDates){
            LocalDateTime ldtToCopy = ldt;
            airDatesCopy.add(ldtToCopy);
        }
        eventCopy.setAirDates(airDatesCopy);

        NavigableMap<LocalDateTime, Auditorium>  auditoriumsCopy = new TreeMap<>();
        for (Map.Entry<LocalDateTime, Auditorium> pair : auditoriums.entrySet()) {
            auditoriumsCopy.put(pair.getKey(),pair.getValue().clone());
        }
        eventCopy.setAuditoriums(auditoriumsCopy);
        return eventCopy;
    }
